...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        newexpr = scheme_eval(first, env)
        check_procedure(newexpr)
        return newexpr.eval_call(rest, env)
        # END PROBLEM 5
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        newoper = operands.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(self, newoper, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        val = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(val, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    elif expressions.first is False:
        return False
    elif expressions.first or expressions.first == 0:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil :
        location = bindings.first
        check_form(location, 2)
        if location.second.second is not nil:
            raise SchemeError('bad bindings list in let form')
        formal = location.first
        value = scheme_eval(location.second.first, env)
        formals = Pair(formal, formals)
        values = Pair(value, values)
        bindings = bindings.second
    check_formals(formals)
    child = env.make_child_frame(formals, values)
    return child
    # END PROBLEM 15
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            val = scheme_eval(first, env)
            check_procedure(val)
            args = rest.map(lambda f : scheme_eval(f, env))
            result = scheme_apply(val, args, env)
            # END PROBLEM 20
    return result
