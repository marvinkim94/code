        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    insect.place = None
                elif hasattr(self.ant, 'truth') and self.ant.unique:
                    pass
                else:
                    self.ant = None
                    insect.place = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and isinstance(self.ant.ant, QueenAnt) and self.ant.ant.unique:
                    pass
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        mover = self.place
        step = 0
        while mover.entrance != 'hive' and mover.entrance :
            if step >= self.min_range and step <= self.max_range :
                if len(mover.bees) != 0 :
                    return random_or_none(mover.bees)
            mover = mover.entrance
            step += 1
        return None
        # END Problem 5
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        newbees = [b for b in self.place.bees]
        self.armor -= amount
        if self.armor <= 0:
            for i in newbees:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.unique :
            ...
            for i in colony.places.values() :
                if i.name < self.place.name and i.ant != None and i.ant not in self.doubled :
                    i.ant.damage *= 2
                    self.doubled.append(i.ant)
                    if isinstance(i.ant, BodyguardAnt) or isinstance(i.ant, TankAnt) :
                        if i.ant.ant and i.ant.ant not in self.doubled :
                            i.ant.ant.damage *= 2
                            self.doubled.append(i.ant.ant)
                elif isinstance(i.ant, TankAnt) and i.ant.ant not in self.doubled :
                    i.ant.ant.damage *= 2
                    self.doubled.append(i.ant.ant)
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def slow_turn(duration) :
        def stop_action(colony) :
            nonlocal duration
            if duration > 0 :
                duration -= 1
                if colony.time%2 != 0 :
