...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self,
        ...
        gameState: A GameState object (pacman.py)    important in question 5
...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        return self.startingPosition, ()  # position and touched corners
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not hitsWall:
                ...
                if next_position in self.corners:  # in a corner now
                    if next_position not in state[1]:
                        new_state = (next_position,
                                     (state[1] + (next_position, ))
...
def cornersHeuristic(state, problem):
    ...
    # # try to use foodheuristic to search
    # totalcost = []
    # # calculate totalcost
    ...
    totalcost = {}
    # initial totalcost
    for i in range(len(untouchedcorners)):
        totalcost[untouchedcorners[i]] = 0
    while (len(totalcost) > 0):
        for corner in totalcost.keys():
            totalcost[corner] = (abs(corner[0] - position[0]) +
                                 abs(corner[1] - position[1]))
        # update the state
        position = min(totalcost, key=lambda key: totalcost[key])
        distance += totalcost[position]
        del totalcost[position]
    return distance
...
def mazeDistance(point1, point2, gameState):
    ...
    Yes,it is!
    If this function is defined earlier, that will be better!  --FYQ
    """
    x1, y1 = point1
    x2, y2 = point2
    walls = gameState.getWalls()
